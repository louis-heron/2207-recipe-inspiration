...
interface IngredientSelectorData {
  options: string[]
  selected: string[]
  version: number
  label: string
...
type IngredientSelectorState = FrontendState & {
  selected: string[] | null
}

interface IngredientSelectorElement extends HTMLElement {
  _riSelected: string[]
  _riVersion: number
  _riInit: boolean
  _riData: IngredientSelectorData
}

const IngredientSelector: FrontendRenderer<IngredientSelectorState, IngredientSelectorData> = ({
  data,
  setStateValue,
  parentElement,
}) => {
  const pe      = parentElement as IngredientSelectorElement
  const labelEl = pe.querySelector<HTMLElement>("#ri-cb-label")!
  const field   = pe.querySelector<HTMLElement>("#ri-cb-field")!
  const chipsEl = pe.querySelector<HTMLUListElement>("#ri-cb-chips")!
  const input   = pe.querySelector<HTMLInputElement>("#ri-cb-input")!
  const listbox = pe.querySelector<HTMLUListElement>("#ri-cb-listbox")!
  ...
  input.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      updateActive(opts)
      if (activeIndex === -1) input.setAttribute("aria-activedescendant", "")
  ...
  document.addEventListener("click", (e) => {
    if (!pe.contains(e.target as Node)) closeListbox()
  })
  ...
  function updateActive(opts: HTMLElement[]): void {
    opts.forEach((o, i) => {
  ...
  function renderListbox(query: string): void {
    ...
    if (listbox.children.length === 0) closeListbox()
  }
