...
const Header = ({ data, setTriggerValue, parentElement, }) => {
    const { logo_svg, logo_alt, logo_page, nav_links, active_page } = data;
    let headerEl = parentElement.querySelector("#app-header");
    if (!headerEl) {
  ...
  `;
    headerEl.querySelectorAll("[data-page]").forEach((a) => {
        a.onclick = (e) => {
            e.preventDefault();
            if (a.dataset.page)
                setTriggerValue("page_clicked", a.dataset.page);
        };
    });
};
